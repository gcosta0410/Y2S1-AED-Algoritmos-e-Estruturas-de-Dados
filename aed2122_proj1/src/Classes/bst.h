...
class BST {
    ...
    friend class iteratorBST<Comparable>;
public:
    int countNodes(BinaryNode<Comparable>* node) const;
...
template <class Comparable>
int BST<Comparable>::countNodes(BinaryNode<Comparable>* node) const{
...
int BST<Comparable>::size(const Comparable& el) const {
    ...
    auto search = find(el, root);
//    BST<Comparable> copy = BST<Comparable>(ITEM_NOT_FOUND);
//    BST<Comparable> copy = BST<Comparable>(clone(*search));
//    copy.copy(BST<Comparable>(search));
    if (search == NULL) return -1;
    else {
        return countNodes(search) - 1;
    }
